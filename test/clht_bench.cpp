...
DEFINE_int32(initsize, 8388608, "initiate buckets");
DEFINE_string(filepath, "/mnt/pmem/", "");
...
public:
    ...
    Benchmark():
        ...
        remove((FLAGS_filepath + "clht_basemd").c_str()); // delete the mapped file.
        remove((FLAGS_filepath + "clht_desc").c_str()); // delete the mapped file.
        remove((FLAGS_filepath + "clht_sb").c_str()); // delete the mapped file.

        hashtable_ = clht_create(FLAGS_initsize);

        barrier_init(&barrier, FLAGS_thread);
    }
    ...
    void DoReadAll(ThreadState* thread) {
        ...
        while (!duration.Done(batch) && key_iterator.Valid()) {
            ...
            for (; j < batch && key_iterator.Valid(); j++) {                          
                ...
                uintptr_t val = clht_get(hashtable_->ht, ikey);
                if (val != ikey) {
                    not_find++;
                }
            }
