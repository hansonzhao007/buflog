...
public:
    ...
    void DoRead (ThreadState* thread) {
        ...
        if (not_find) INFO ("thread %2d num: %lu, not find: %lu\n", thread->tid, reads_, not_find);
    ...
    void PrintTree (ThreadState* thread) {
        auto res = tree_->ToString ();
        std::ofstream file ("tree.data");
        file << res;
        file.close ();
        return;
    }
